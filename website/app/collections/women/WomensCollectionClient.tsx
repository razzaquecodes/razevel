...
export default function WomensCollectionClient() {
  ...
  const baseProducts = useMemo(() => MOCK_PRODUCTS.filter(p => p.gender === 'women'), []);
    ...
    <main style={{ background: 'var(--luxury-white)', minHeight: '100vh', position: 'relative' }}>
      ...
      <section style={{ position: 'relative', height: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <Image src="/images/journal.png" alt="Womens Collection" fill style={{ objectFit: 'cover' }} priority />
      ...
      <section style={{ padding: 'clamp(4rem, 8vw, 8rem) var(--gutter)', textAlign: 'center', maxWidth: 'var(--max-w-narrow)', margin: '0 auto' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 3vw, 2.5rem)', color: 'var(--color-black)', margin: '0 0 1.5rem' }}>
          Heirloom <em style={{ color: 'var(--color-gold)' }}>Couture.</em>
        </h2>
        <p className="t-body-lg">
          Bridal lehengas that span generations. Pakistani suits with unparalleled Zardozi. Our women's collection is a testament to the enduring power of feminine grace and uncompromising luxury craftsmanship.
        </p>
      </section>
              ...
              <>
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', overflowY: 'auto', flex: 1, paddingRight: '0.5rem' }}>
                    ...
                    <FilterGroup 
                      title="Occasion" 
                      options={['Wedding', 'Reception', 'Festive', 'Casual']} 
                      selected={selectedOccasions}
                      onChange={handleOccasionChange}
                    />
                    <FilterGroup 
                      title="Fabric" 
                      options={['Banarasi Silk', 'Chiffon', 'Organza', 'Velvet']} 
